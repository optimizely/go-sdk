...
func checkSubset(expected, target reflect.Value, visited map[uintptr]*visit, depth int) (b bool) {
	...
	default:
		...
		result := reflect.DeepEqual(expectedInterface, targetInterface)
		return result
	}
}
...
func logDifference(expected, actual reflect.Value) {
	fmt.Printf("\nType() differs expected: %s actual: %s", reflect.TypeOf(expected.Interface()), reflect.TypeOf(actual.Interface()))
	fmt.Printf("\nValue expected: %s actual: %s", expected.Interface(), actual.Interface())
}

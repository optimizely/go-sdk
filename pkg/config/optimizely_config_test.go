...
func (s *OptimizelyConfigTestSuite) TestOptlyConfigIncludesHoldouts() {
	// Test with minimal datafile (no holdouts)
	datafile := []byte(`{"version":"4"}`)
	projectMgr := NewStaticProjectConfigManagerWithOptions("", WithInitialDatafile(datafile))
	optimizelyConfig := NewOptimizelyConfig(projectMgr.projectConfig)

	// Verify holdouts field exists and is empty
	s.NotNil(optimizelyConfig.Holdouts)
	s.Empty(optimizelyConfig.Holdouts)
	...
	holdouts, exists := jsonMap["holdouts"]
	s.True(exists, "holdouts field should exist in JSON")
	s.IsType([]interface{}{}, holdouts, "holdouts should be an array")
}

func (s *OptimizelyConfigTestSuite) TestOptlyConfigWithHoldouts() {
	// Test with datafile containing holdouts
	rootDirectory := "testdata/"
	dataFile, err := os.ReadFile(rootDirectory + "holdouts_datafile.json")
	if err != nil {
		s.Fail("error opening holdouts_datafile.json")
	}
	...
	// Verify holdouts are present
	s.NotNil(optimizelyConfig.Holdouts)
	s.Len(optimizelyConfig.Holdouts, 4, "Should have 4 holdouts")

	// Verify holdout structure
	expectedHoldoutKeys := map[string]bool{
		"holdout_3":  false,
		"holdout_5":  false,
		"holdouts_4": false,
		"holdout_6":  false,
	...
	for _, holdout := range optimizelyConfig.Holdouts {
		// Verify required fields
		s.NotEmpty(holdout.ID)
		s.NotEmpty(holdout.Key)
		s.NotEmpty(holdout.Audiences)
		s.NotNil(holdout.VariationsMap)

		// Mark key as found
		if _, exists := expectedHoldoutKeys[holdout.Key]; exists {
			expectedHoldoutKeys[holdout.Key] = true
		}

		// Verify variation structure
		s.Contains(holdout.VariationsMap, "off", "Holdout should have 'off' variation")
		offVariation := holdout.VariationsMap["off"]
		s.Equal("$opt_dummy_variation_id", offVariation.ID)
		s.Equal("off", offVariation.Key)
		s.False(offVariation.FeatureEnabled)
	}

	// Verify all expected holdout keys were found
	for key, found := range expectedHoldoutKeys {
		s.True(found, "Holdout with key %s should be present", key)
	}
	...
	holdoutsJSON, exists := jsonMap["holdouts"]
	s.True(exists)
	holdoutsArray, ok := holdoutsJSON.([]interface{})
	s.True(ok)
	s.Len(holdoutsArray, 4)
}
